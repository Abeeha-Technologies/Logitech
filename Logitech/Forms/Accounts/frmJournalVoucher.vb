...
Public Class frmJournalVoucher
    Const VoucherType As String = "JV"
    Const MasterTable As String = "tblAccJournalVoucherMaster"
    Const DetailTable As String = "tblAccJournalVoucherDetail"
    ...
    Sub ClearDetailFields()
        ...
        txtDebitDetail.Text = 0
        txtCreditDetail.Text = 0
    ...
    Private Sub frmPaymentVoucher_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        SetformStateNew()
        cboVoucherTypeSearch.SelectedItem = "JV"
    ...
    Private Sub frmPaymentVoucher_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles MyBase.KeyDown
        ...
        '        dgv_KeyDown(dgv, e)
        '    ElseIf dgvFixedSlab.Focused = True Then
        '        dgvFixedSlab_KeyDown(dgv, e)
        '    Else
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        If dgvTransactions.Rows.Count < 1 Then
            ...
            Exit Sub
        End If
        If Val(txtTotalDifference.Text) <> 0 Then
            MsgBox("Debit And Credit Must be Same")
            Exit Sub
        End If
        Dim cmd As New SqlCommand
        ...
        If txtIsEdit.Text = Nothing Then
            Try
                With cmd
                    .CommandText = "INSERT INTO " & MasterTable & " (VoucherId, VoucherNo, VoucherType, VoucherDate, EntryUser, IsApproved) VALUES (@VoucherId, @VoucherNo, @VoucherType, @VoucherDate, @EntryUser, @IsApproved)"
                    .Parameters.Clear()
                ...
                If dgvTransactions.Rows.Count > 0 Then
                    For Each row As DataGridViewRow In dgvTransactions.Rows
                        With cmd
                            .CommandText = "INSERT INTO " & DetailTable & " (VoucherId, AccCode, Description, Debit, Credit) VALUES (@VoucherId, @AccCode, @Description, @Debit, @Credit)"
                            .Parameters.Clear()
                            .Parameters.Add("@VoucherId", SqlDbType.NVarChar).Value = txtVoucherId.Text
                            ...
                            .Parameters.Add("@Debit", SqlDbType.Decimal).Value = row.Cells("Debit").Value
                            .Parameters.Add("@Credit", SqlDbType.Decimal).Value = row.Cells("Credit").Value
                            .ExecuteNonQuery()
                        End With
                    Next
                End If
        ...
        If txtIsEdit.Text <> Nothing Then
            Try
                With cmd
                    .CommandText = "UPDATE " & MasterTable & " SET VoucherNo = @VoucherNo, VoucherType = @VoucherType, VoucherDate = @VoucherDate, EntryUser = @EntryUser, IsApproved = @IsApproved WHERE VoucherId = @VoucherId"
                    .Parameters.Clear()
                    .Parameters.Add("@VoucherId", SqlDbType.NVarChar).Value = txtVoucherId.Text
                    .Parameters.Add("@VoucherNo", SqlDbType.BigInt).Value = nudVoucherNumber.Value
                    .Parameters.Add("@voucherType", SqlDbType.NVarChar).Value = cboVoucherType.SelectedItem
                ...
                If dgvTransactions.Rows.Count = 0 Then
                    ...
                    qryDelete = "DELETE FROM " & DetailTable & " WHERE VoucherId = '" & txtVoucherId.Text & "'"
                    With cmdDelete
                        .Transaction = trans
                        .Connection = Conn
                        .CommandType = CommandType.Text
                ...
                For i = 0 To dgvTransactions.Rows.Count - 1
                    ...
                    If Val(dgvTransactions.Rows(i).Cells("DetailId").Value) > 0 Then
                        With cmd
                            .CommandText = "UPDATE " & DetailTable & " SET AccCode = @AccCode, Description = @Description, Debit = @Debit, Credit = @Credit WHERE DetailId = @DetailId"
                            .Parameters.Clear()
                            ...
                            .Parameters.Add("@Debit", SqlDbType.Decimal).Value = dgvTransactions.Rows(i).Cells("Debit").Value
                            .Parameters.Add("@Credit", SqlDbType.Decimal).Value = dgvTransactions.Rows(i).Cells("Credit").Value
                            .ExecuteNonQuery()
                        End With
                    End If
    ...
    Private Sub btnSearchAccMaster_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
    ...
    Sub ShowAccSearchDialog(ByVal Source As String)
        ...
        If Source = "Detail" Then
            ...
            InitialValue1 = "JournalVoucherDetail"
    ...
    Private Sub txtTotalDebit_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs)
        If Val(txtTotalDebit.Text) > 0 Then

            lblResult.Text = NumberToText(txtTotalDebit.Text) & " Rupees Only"
        Else
    ...
    Sub FillTransactions()
        ...
        With cmd
            .Connection = Conn
            .CommandText = "SELECT dbo.tblAccJournalVoucherDetail.DetailId, dbo.tblAccJournalVoucherDetail.VoucherId, dbo.tblAccJournalVoucherDetail.AccCode, dbo.tblAccJournalVoucherDetail.Description, dbo.tblAccJournalVoucherDetail.Debit, dbo.tblAccJournalVoucherDetail.Credit, dbo.tblAccJournalVoucherDetail.SysTimeStamp, dbo.tblAccChartOfAccounts.AccName FROM dbo.tblAccJournalVoucherDetail INNER JOIN dbo.tblAccChartOfAccounts ON dbo.tblAccJournalVoucherDetail.AccCode = dbo.tblAccChartOfAccounts.AccCode WHERE VoucherId = @VoucherId"
            .Parameters.Clear()
            .Parameters.Add("@VoucherId", SqlDbType.NVarChar).Value = txtVoucherId.Text
        End With
        dt.Clear()
        ...
        If dt.Rows.Count > 0 Then
            ...
            For Each row As DataRow In dt.Rows
                dgvTransactions.Rows.Add(row.Item("AccCode"), row.Item("AccName"), row.Item("Description"), row.Item("Debit"), row.Item("Credit"), row.Item("DetailId"))
    ...
    Private Sub txtAccCodeDetail_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtAccCodeDetail.TextChanged
        ...
        GetAccDetail("Detail", Trim(txtAccCodeDetail.Text), "AccCode = @AccCode ")
    End Sub
    ...
    Sub CalcTotal()
        ...
        TotalCredit = 0
        For i = 0 To dgvTransactions.Rows.Count - 1
            TotalDebit += dgvTransactions.Rows(i).Cells("Debit").Value
            TotalCredit += dgvTransactions.Rows(i).Cells("Credit").Value
        Next
        txtTotalDebit.Text = TotalDebit
        txtTotalCredit.Text = TotalCredit

        txtTotalDifference.Text = TotalDebit - TotalCredit
    End Sub
    ...
    Private Sub txtDebitDetail_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txtDebitDetail.KeyDown
        If e.KeyCode = Keys.Enter Then
            txtCreditDetail.Focus()
        End If
    End Sub
    Private Sub txtCreditDetail_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txtCreditDetail.KeyDown
        If e.KeyCode = Keys.Enter Then
    ...
    Private Sub btnPin_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnPin.Click
        If txtAccTitleDetail.Text = Nothing Then
            ...
            Exit Sub
        End If
        If Val(txtDebitDetail.Text) <= 0 And Val(txtCreditDetail.Text) <= 0 Then
            MsgBox("Please Enter Valid Debit or Credit")
            Exit Sub
        End If
        If Val(txtDebitDetail.Text) > 0 And Val(txtCreditDetail.Text) > 0 Then
            MsgBox("Debit and Credit can not be entered in same transaction")
            Exit Sub
        End If
        If txtDetailOrderRowIndex.Text = Nothing Then
            dgvTransactions.Rows.Add(txtAccCodeDetail.Text, txtAccTitleDetail.Text, txtDescriptionDetail.Text, txtDebitDetail.Text, txtCreditDetail.Text, "")
        ...
        Else
            For Each row As DataGridViewRow In dgvTransactions.Rows
                If row.Index = txtDetailOrderRowIndex.Text Then
                    ...
                    row.Cells("Debit").Value = txtDebitDetail.Text
                    row.Cells("Credit").Value = txtCreditDetail.Text
                End If
    ...
    Sub FillTransactionDetail()
        If dgvTransactions.Rows.Count > 0 Then
            With dgvTransactions.CurrentRow
                ...
                txtDebitDetail.Text = .Cells.Item("Debit").Value
                txtCreditDetail.Text = .Cells.Item("Credit").Value
            End With
    ...
    Private Sub txtAccCodeMaster_KeyPress(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyPressEventArgs)
    ...
    Private Sub pnlAccountSearch_VisibleChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles pnlAccountSearch.VisibleChanged
        If pnlAccountSearchSource = "DetailTitle" Then
    ...
    Private Sub SelectAccount(ByVal Source As String)
        If Source = "DetailTitle" Or Source = "DetailCode" Then
    ...
    Private Sub txtAccCodeMaster_Enter(ByVal sender As System.Object, ByVal e As System.EventArgs)
        pnlAccountSearch.Location = New Point(82, 32)
    End Sub

    Private Sub txtAccTitleMaster_Enter(ByVal sender As System.Object, ByVal e As System.EventArgs)
        pnlAccountSearch.Location = New Point(82, 32)
    End Sub
    ...
    Private Sub chkIsApproved_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles chkIsApproved.CheckedChanged
        If chkIsApproved.Checked = True Then
            btnApproveToggle.Image = Logitech.My.Resources.Resources.Png_CircleRed_24px
            btnApproveToggle.Text = "Draft"
        Else
            btnApproveToggle.Image = Logitech.My.Resources.Resources.Png_CircleGreen_24px
